...
**TinaCMS — Phase 0 done, paused.** Not urgent, not abandoned. Plan saved at `~/.claude/plans/is-there-a-way-frolicking-hammock.md`. Scaffolding sits inert in the repo.
...
- **Two GHL workflows need to actually be created in the GHL dashboard** (both documented step-by-step in `docs/ghl-layer-0.md`, neither needs code): (1) the Vercel Deploy Hook workflow (§7) so a GHL change goes live without a manual redeploy, (2) the reviews-webhook workflow (§ "Auto-backfilling Google ratings on upgrade") so a newly-paid listing gets its rating without running `npm run reviews` by hand. `REVIEWS_WEBHOOK_SECRET` also needs to be set in `.env` and Vercel.
