...
**The site is live on Vercel**, imported from GitHub (`github.com/MarzLeBran/Central-FL-Vybe`, `main` branch), with production env vars set (`DATA_SOURCE=ghl`, `GHL_PIT_TOKEN`, `GHL_LOCATION_ID`) — confirmed actually reading live GHL data, not mock. First deploy failed silently (see the TinaCMS entry below) — fixed and confirmed working from a genuinely fresh clone, not just local. Currently serving on Vercel's default `*.vercel.app` domain; the purchased custom domain is bought but deliberately not connected yet.

**GHL live-sync is fully working and confirmed end-to-end, both directions**: a GHL workflow ("Directory Live Sync," two triggers — Contact Tag Added and Contact Tag Removed, both filtered to `business`) POSTs to a Vercel Deploy Hook on either event, triggering an automatic rebuild — no manual redeploy needed anymore. Real gotcha hit and resolved along the way, worth remembering for next time something "should be live but isn't": **editing an already-published GHL workflow leaves the live version frozen at whatever was last published** — new/changed triggers sit in an unpublished draft until you explicitly hit Publish again. That's why "Tag Removed" worked immediately but "Tag Added" silently did nothing for a while — only the removed-trigger version had ever been published. The reviews-webhook workflow (separate automation) is still intentionally not set up — deferred until the custom domain is connected, see Next.
...
- **Almost all 138 sourced businesses are currently untagged (`business` removed) on purpose** — the user wants to hand-pick which of them go live rather than auto-publish all of them; only a couple were re-tagged as live-sync tests. Don't be alarmed that the live site shows few/no listings — that's the current intentional state, not a bug. Real go-forward plan: solicit businesses via Facebook posts pointing at `/add-business` (already fully built — lands as `new_business_request`, invisible until manually reviewed and tagged `business`), plus hand-picking from the existing 138 as desired.
...
- **Confirm `AUTH_SECRET`, `BLOB_READ_WRITE_TOKEN`, `ADMIN_PASSWORD` are also set in Vercel** (only `DATA_SOURCE`/`GHL_PIT_TOKEN`/`GHL_LOCATION_ID` are confirmed set so far) — needed for owner/admin editing and photo uploads to work on the live site, not just locally.
